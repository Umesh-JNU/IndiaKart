...
const Products = () => {
    ...
    const {products, loading, error, productsCount, itemPerPage, filteredProductsCount} = useSelector(
    ...
    }, [dispatch, keyword, curPage, price, category, ratings, error, alert]);

    const count = filteredProductsCount;
    const noPages = parseInt(productsCount / itemPerPage) + (productsCount % itemPerPage === 0 ? 0 : 1);
    return (
        <>{loading ? <Loading /> : 
            <>
                ...
                {itemPerPage < count && ( <Pagination count={noPages} page={curPage} variant="outlined" color="primary" siblingCount={0} boundaryCount={2} showFirstButton showLastButton onChange={setCurPageNo} />)}
            </>}
        </>
    )
}
